const KEY = 'eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiJ9..EZJsuRbBPxtASDJJr59IJSTa5kAdycLcBMkbD7wlj6-yy_ow4TFSR4DUNRLZo1nm2N_LrfI8qrLdGExUYIXv1I3W_MTiqhCJxXI7r_B6YNqOnceXRuYvh1LBMhOCB8_pkZ9_9SCAesoRrGGZzb7_80Kvuwjp45nGaKODDjpl8MUSs-KzhvIv6Aw290O8IoH2yEYdt2BCfbKWYgZZOeAGsXaayhaQtSYMsZVza54J4tPIB8HGIQLVRmBsC3Ikog2TpkK8klhaWbFGgu3UzJlf47aD33THSo9eB2PHnzfUbHfK4PtgRzSevsUPrDKiWEv37HCf8vf-Q3kAWHjSw6V4njEhV-hljypn7dPr8l2nmyQrlEOBrI9klF5r-jRid4f5XlpT_5bm88QLC5xNJ_HnMoDXQnj0K2nVE24y7nlnMbqQ8o_yUZ_xPrOAkJwdUHJTbmJ1oz0_nlpUu5MUqvasQrc09oWO3UsDpc-ud1HtvzL18jw4OX1HGoBH4xdrpncy-d4im8o1pbKfwi-6iAB3SpOuOS4O8-HYbPXJ-vtn2pY8iPZ1VAOpar206wwx15shezNqtwMphjwZVfS-rhIMrb6puMD_egaVJeKoLjN65YYMc6thycJHTdmYl-dk7I_sLZMcAksa0WXwJAHyVTCCbXykTEkUmduPHXKDxtBUj3Q';
const BASE = 'https://connect.mailerlite.com/api';

async function api(path, method, body) {
  const opts = {
    method: method || 'GET',
    headers: { 'Authorization': `Bearer ${KEY}`, 'Content-Type': 'application/json', 'Accept': 'application/json' }
  };
  if (body) opts.body = JSON.stringify(body);
  const res = await fetch(`${BASE}${path}`, opts);
  const text = await res.text();
  try { return { status: res.status, data: JSON.parse(text) }; } catch { return { status: res.status, data: text }; }
}

async function main() {
  // Step 1: Get automation details
  console.log('=== Step 1: Get automation ===');
  const auto = await api('/automations');
  console.log('Status:', auto.status);
  if (auto.status === 200) {
    console.log('Automations:', JSON.stringify(auto.data.data?.map(a => ({id: a.id, name: a.name, enabled: a.enabled})), null, 2));
    // Find EN automation
    const enAuto = auto.data.data?.find(a => a.name?.includes('EN Lead Magnet'));
    if (enAuto) {
      console.log('\nEN Automation ID:', enAuto.id);
      console.log('Steps:', JSON.stringify(enAuto.steps?.map(s => ({id: s.id, type: s.type, name: s.name, email_id: s.email_id})), null, 2));
      
      // Step 2: Get the email campaign for email step
      if (enAuto.steps) {
        for (const step of enAuto.steps) {
          if (step.type === 'email' && step.email_id) {
            console.log(`\n=== Step 2: Get email campaign ${step.email_id} ===`);
            const camp = await api(`/campaigns/${step.email_id}`);
            console.log('Status:', camp.status);
            console.log('Campaign data:', JSON.stringify(camp.data).substring(0, 800));
          }
        }
      }
    }
  } else {
    console.log('Error:', auto.data);
  }
}

main().catch(console.error);
