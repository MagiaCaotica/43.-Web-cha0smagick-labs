...
**Causa raíz CONFIRMADA, y con dos hipótesis refutadas por el camino.** La atribución se resolvió por medición, no por inspección. Primero quedaron descartadas dos suposiciones: (1) **las imágenes NO son la causa** — `unsized-images` pasa con **0 ítems** y todas las imágenes terminan de cargar antes de 1.2 s; (2) **el swap de webfont NO es la causa** — `index.html` sí carga Google Fonts con `display=swap`, pero la ablación decisiva (bloquear `fonts.googleapis.com` y `fonts.gstatic.com` y volver a medir) dio **CLS 0.779 en el control contra 1.058 con las fuentes bloqueadas**: el CLS no bajó, subió. Esa hipótesis queda **descartada**.

**La causa medida:** `js/conversion.js` construye el popup de salida en `init()` (`buildPopup()` en L1359) y hace `host.innerHTML = '<div id="cm-email-popup" class="cm-popup-overlay" ...>'` (L1313) sobre el placeholder `<div id="exit-intent-popup"></div>` que `index.html` ya trae. **`css/style.css` no tiene ni una regla para `#exit-intent-popup` ni para `.cm-popup-overlay`** (0 coincidencias de cada una), y el CSS que las posiciona se inyecta en `<head>` en runtime, con `document.head.insertAdjacentHTML('beforeend', css)` (L2275). Resultado: en escritorio el overlay entra en el DOM como bloque en flujo **antes** de tener el CSS que lo convierte en `position:fixed`, así que añade altura real al final de `<body>` y reflowea el documento entero. Eso es exactamente la firma medida: el mayor desplazamiento individual cae sobre **`body`** (0.42–0.47) y le siguen varios sobre `body > main#home`.

**El experimento natural que lo confirma:** el disparador de salida está condicionado a escritorio por `if (isDesktop())` (L1399, `matchMedia('(min-width: 1024px)')`). En móvil el popup nunca se construye, y el móvil mide **CLS 0.000 exacto en todas las corridas**, mientras el escritorio mide 0.78–1.24. Misma página, mismo código, diferencia de comportamiento exactamente en la rama del popup. Queda pendiente un solo paso de confirmación: deshabilitar `buildPopup()` y volver a medir, que corresponde a **P1-01** junto con el arreglo — y cuyo gate de dependencia (P0-10) sigue abierto, así que aquí se registra y **no se arregla**.
