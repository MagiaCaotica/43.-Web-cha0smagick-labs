...
**Defecto reproducible, no artefacto de laboratorio:** el CLS de escritorio se midió en 8 corridas independientes — 0.961, 1.115, 0.932, 0.920, 1.235, 1.1407, 0.7788 y 1.0578 — mientras el móvil mide **exactamente 0.000** en todas las corridas. Lo que es robusto es la **magnitud**: entre 0.78 y 1.24, es decir de 8 a 12 veces el umbral de 0.1, siempre en un orden de magnitud por encima. Lo que **no** es estable es el valor exacto, así que la varianza de laboratorio importa y comparar dos corridas sueltas no prueba nada. `performance` de escritorio se mueve entre 36 y 53. Es un defecto **solo de escritorio**, real y medible. `accessibility` (91), `best-practices` (96) y `seo` (100) son estables en todas las corridas.
...
**Causa raíz CONFIRMADA por medición, y con tres hipótesis refutadas por el camino.** La atribución se resolvió con una sonda nueva, `scripts/analytics/cls-shift-probe.mjs`, que inyecta un `PerformanceObserver` de `layout-shift` vía CDP `Page.addScriptToEvaluateOnNewDocument` y reporta, por cada desplazamiento, el nodo atribuido con su `previousRect`/`currentRect` y su cadena de ascendencia. (Puppeteer 25 eliminó `evaluateOnNew*` de `Page.prototype`; la sonda usa la sesión CDP directa.) Medido contra producción el 2026-09-25: **CLS 1.09 en escritorio con 8 entradas; 0.000 con 0 entradas en móvil**, y la firma es `body` yendo de `y8 h932 x7 w1335` a `y0 h940 x0 w1350` a los ~630 ms. Eso es el **margen de 8 px por defecto del user-agent presente en el primer pintado y eliminado después**: la firma de *una página que se pintó antes de que le aplicaran la hoja de estilos*, no de un elemento que crece. Descartadas: (1) **las imágenes NO son la causa** — `unsized-images` pasa con **0 ítems** y todas las imágenes terminan de cargar antes de 1.2 s; (2) **la barra de desplazamiento NO es la causa** — `innerWidth` y `clientWidth` valen 1350 en todas las entradas y `scrollbar_present` es `false` siempre; (3) **el swap de webfont NO es la causa** — la ablación (bloquear `fonts.googleapis.com` y `fonts.gstatic.com` y volver a medir) dio **CLS 0.779 en el control contra 1.058 con las fuentes bloqueadas**: el CLS no bajó, subió.

**Lección de método — una ablación nula es indistinguible de un resultado si no se instrumenta.** Al intentar repetir esas ablaciones con la sonda nueva, `--block /css/style.css` resultó en `blocked_request_count: 0`: el archivo servido es **`css/style.min.css`**, no `style.css`, así que el patrón no coincidió con nada y la corrida no probaba nada. Por eso la sonda reporta `blocked_patterns`, `blocked_request_count` y `blocked_sample` en su salida: **una ablación solo es evidencia si primero se demuestra que efectivamente bloqueó algo.** Sin ese campo, un nombre de archivo equivocado se lee como "el bloqueo no cambió nada, luego la causa es otra", que es exactamente el modo de fallo que hizo parecer inocente al popup.

**Hipótesis REFUTADA — el popup de salida era un señuelo.** Una versión anterior de este documento afirmaba que `js/conversion.js` construye el popup en `init()` y que el overlay entra al DOM como bloque en flujo. **Es falso, y se refuta leyendo el código:** `buildPopup()` (L1298) **no se llama desde `init()`**; solo es alcanzable desde `showPopup()` (L1359), y `showPopup()` solo desde un disparador. `CONFIG.popupDelayMs = 30000` (L48) y el único temporizador es `setTimeout(fire, CONFIG.popupDelayMs)` (L1383), así que **una corrida de laboratorio nunca llega a los 30 s y el popup nunca se construye durante el audit**. Y aunque se construyera: `index.html:855` trae `<div id="exit-intent-popup" aria-hidden="true" style="display:none;">` —con `display:none` en línea que `buildPopup()` nunca quita— y el CSS inyectado en runtime declara `#exit-intent-popup { display: none; }` (L2030-2032) y `.cm-popup-overlay { position: fixed; inset: 0; z-index: 99999; … }` (L2033-2043), o sea que **no puede añadir altura al documento**. Que la ausencia total de reglas para `#exit-intent-popup` y `.cm-popup-overlay` en `css/style.css` sea cierto no convertía la hipótesis en causa: esas reglas se inyectan en `<head>` en runtime con `document.head.insertAdjacentHTML('beforeend', css)` (L2275), y un componente que se posiciona por CSS runtime no necesita reglas en la hoja estática para funcionar. El razonamiento que estaba mal no era ese dato, sino el salto de "no hay CSS para él" a "entra sin estilo y empuja el layout".

**La causa medida:** el patrón anti-CSS asíncrono. `index.html:61-62` traía
`<link rel="preload" href="css/style.min.css" as="style" onload="this.rel='stylesheet'">` seguido de `<noscript><link rel="stylesheet" href="css/style.min.css"></noscript>`.
`rel="preload" as="style"` combinado con `onload="this.rel='stylesheet'"` permite que el navegador **pinte antes de aplicar la hoja de estilos** y después reestilice y reflowee el documento entero. Eso explica la firma completa de una sola causa: el `body` pierde su margen de 8 px, los `<li>` del nav crecen de forma monótona en alto **y** ancho (Inter llegando por el `<link rel="stylesheet">` render-blocking de Google Fonts en `index.html:13-14`), el `header` crece, y `main#home` se desplaza ~82 px hacia abajo, con `div.hero-cta-row` empujado fuera del viewport. El repositorio **ya tenía la forma correcta** en `tools/index.html:37-38`; lo que se corrigió fue la inconsistencia interna, no un criterio nuevo.

**Aritmética del repo, verificada sobre los 622 HTML del sistema de archivos:** exactamente **371 archivos** usaban el patrón y existía **una sola variante** de cada construcción — 371 con su `<noscript>` emparejado y 0 sin él, y 523 `<noscript>` en total, o sea que los 152 restantes ya tenían un `<link rel="stylesheet">` de verdad cuyo `<noscript>` es inerte y no se tocó. El arreglo es adoptar la forma que el repo ya usaba en `tools/index.html`: `<link rel="stylesheet" href="H">`. Se escribió `scripts/fix-async-css.mjs`, con dry-run por defecto, `--apply`, y dos invariantes que abortan la corrida si un archivo deja un `preload as="style"` o si acaba con **el mismo** `href` de hoja de estilos dos veces. La segunda invariante existe porque `blog/pendulum-divination-beginners.html` carga legítimamente **dos** hojas distintas (`../styles.css` y `../css/style.min.css`): la primera versión de la aserción exigía exactamente un `<link rel="stylesheet">` y falló contra un archivo correcto, así que la aserción correcta es "no dupliques `href`", no "deja un solo link". Un invariante que se dispara sobre un archivo sano es un invariante mal escrito, no un archivo malo.

**Prueba local antes de escalar — el orden es el que hace la prueba válida:** 1) servidor estático local, 2) línea base, 3) arreglo en `index.html` **solo**, 4) volver a medir, 5) recién entonces escalar a los 371. `index.html` pasó de **1.09 a 0.000 con 0 entradas, 3 de 3 corridas**, y la firma de producción se reprodujo localmente de forma idéntica *antes* de tocar nada, lo que confirma que el banco de pruebas local es fiel. Un dato que evita una afirmación falsa: `blog/index.html` medía **0.000 incluso sin el arreglo**, así que la magnitud del defecto es **dependiente de plantilla** — no toda página que lleva el patrón sufre CLS, y no puede afirmarse "CLS 0 en las 371" sin medir. Los artículos de blog, en cambio, sí fallaban: 0.11, 0.26 y 0.26. Tampoco había contaminación de caché entre corridas: `puppeteer.launch()` sin `userDataDir` crea un perfil temporal nuevo por corrida, así que el disco de caché arranca vacío cada vez.

**Cierre verificado el 2026-09-26 — el defecto de Lighthouse quedó ARREGLADO, no registrado.** 371 archivos corregidos en disco, de los cuales **370 versionados**: la diferencia es `checklist-ventas.html`, que está en `.gitignore:33` y por eso no aparece en `git status`. Post-arreglo, las cinco páginas medidas dan **CLS 0.000 y 0 entradas** en escritorio (antes 1.09 / 0 / 0.11 / 0.26 / 0.26, con 3 de 5 sobre el umbral) y **0.000 también en móvil**, sin regresión. El resto de la batería, idéntica a la línea base: `verify_tech_debt.py` 569 páginas y **0 errores de validación** con los mismos conteos de entidades y de imágenes; `npm test` **38 pytest + 229 Vitest en 11 archivos**, exit 0; `npm run build` exit 0; `thin_articles_report.py` 467 artículos, 0 pendientes. Los diffs son de una línea por archivo, exactamente la transformación prevista. Sobrevive un `rel="preload" as="style"` en `index.html`, y es **otro** patrón: `<link rel="preload" as="style" href="https://fonts.googleapis.com/…">` sin `onload` y con el orden de atributos invertido, un preload inerte que nunca llega a ser hoja de estilos; no causa CLS y no se tocó. **Pendiente para el owner, no bloqueante:** `blog/witchcraft-for-beginners-guide.html` tiene `og:image` y `twitter:image` apuntando a `https://www.cha0smagick.com/img/blog/witchcraft-beginners-guide.webp`, y ese archivo **no existe** en el repositorio (sí existe `assets/images/Banner.png`, que es lo que usa el resto del sitio). No se sustituyó en silencio porque cambiar la imagen social de un artículo es una decisión de contenido, no una corrección de bug.

**Lo que sigue SIN cerrar y no es remediable localmente:** el gate de Lighthouse en el dominio **desplegado** no se ha vuelto a correr, porque el arreglo todavía no está en producción. La medición local de 0.000 es la evidencia del arreglo; la evidencia del gate cerrado exige `npm run build` + `npm test` + `scripts/analytics/lighthouse-audit.mjs` **contra `https://cha0smagicklabs.com` después del deploy**, y eso es P0-10. El `TBT` de móvil (216–236 ms contra un umbral de 200 ms) y el `LCP` de escritorio (2769 ms) **no los toca este arreglo** y siguen abiertos como observación borderline; `performance` de escritorio debería recuperarse al arreglarse el CLS, pero eso también hay que medirlo, no suponerlo.
...
| P0-10 | `IN_PROGRESS` | **2026-09-26: el defecto de CLS de escritorio está ARREGLADO y verificado en local (ver §11.2). El gate sigue abierto porque el arreglo aún no está en producción: falta deploy + re-auditar Lighthouse contra el dominio real.** | `npm test` y `npm run build` pasan (38 pytest + 229 Vitest en 11 archivos); thin report 467/467; tech debt 0 errores sobre 569 páginas. **Smoke del dominio real ejecutado 2026-09-25** (HTTPS `GET` de solo lectura contra `https://cha0smagicklabs.com`): las **568 URLs canónicas responden 567 `200` directo + 1 vía `301` desde `www`, con 0 `404` y 0 inalcanzables**; `robots.txt` declara el sitemap; el sitemap servido tiene 532 entradas idénticas al local; el reparto 15/38 del set excluido queda confirmado en producción (15 rastreados → `200`, 38 no rastreados → `404`); `checklist-ventas.html` → `404`, confirmando que gitignore impide su despliegue. **Consentimiento verificado con navegador real 2026-09-25** vía `scripts/analytics/consent-browser-test.mjs` (Puppeteer, Chrome 151) contra el dominio desplegado: **PASS, exit 0, 30 checks, 0 fallos, 0 inconclusos**; control negativo (URL inexistente) devuelve **INCONCLUSIVE, exit 2** por `HTTP 404`, lo que prueba que el test es capaz de fallar. Medido: primera visita sin cookie → `isConsentGranted()=true` y el primer `gtag('consent','update')` es `granted`; Decline → cookie `declined` + update `denied` en los 4 campos + `isConsentGranted()=false` + `track('tool_start')` rechazado, estable tras recarga; Accept → espejo con `track()` aceptado. ID de GA4 capturado: `G-V6LHCPN9TK`. **Lighthouse ejecutado 2026-09-25** vía `scripts/analytics/lighthouse-audit.mjs` (`lighthouse@13.5.0` instalado como devDependency) con Chrome real vía Puppeteer: **móvil FAIL exit 1** (LCP 2210 ms OK, CLS 0.000 OK, **TBT 216 ms** por encima del umbral; performance 93) y **desktop FAIL exit 1** (**LCP 2769 ms** y **CLS 0.961**; performance 45), con `accessibility` 91, `best-practices` 96 y `seo` 100 estables en todas las corridas, y control negativo validado (**INCONCLUSIVE exit 2** por `HTTP 404`) El CLS de escritorio se midió en 8 corridas (0.78–1.24, siempre 8–12× el umbral) mientras el móvil mide 0.000 exacto. **Causa raíz CORREGIDA el 2026-09-26 (esta celda decía antes que el popup de salida, construido en `init()`, entraba en flujo y refloweaba el documento; es FALSO y está refutado en §2.9.3 — la causa real es el patrón async-CSS `rel="preload" as="style"` en 371 archivos):** Quedaron **refutadas** por medición dos hipótesis: las imágenes (`unsized-images` pasa con 0 ítems) y el swap de webfont (ablación: CLS 0.779 control contra 1.058 con las fuentes bloqueadas) | La rama de **withdrawal** del consentimiento (sin banner) no está cubierta por el test; confirmación en **GA4 DebugView** de que el `denied` evita el envío (o de que no lo evita) — el test solo prueba el lado cliente, nunca la ingesta; **datos de campo de CWV**: PageSpeed Insights sin API key devolvió `HTTP 429` con `limit 'Queries per day'` y `"quota_limit_value": "0"` — **cuota diaria cero, bloqueo duro, no transitorio**; reintentar no puede funcionar y sin tráfico suficiente tampoco habría CrUX, así que `cwvComplete:false` y **INP sigue sin medirse** (es immeasurable en laboratorio). Obtener CWV de campo **exige un proyecto de Google Cloud con API key y la API de PageSpeed Insights habilitada**: acción de owner, no reintento; **TBT de móvil** entre 216 y 236 ms según la corrida, justo sobre el umbral de 200 ms (borderline, no estable); **el arreglo del CLS no se ha aplicado** — es trabajo de P1-01 y su gate (P0-10) sigue abierto, y el paso final que aquí se pedía —deshabilitar `buildPopup()` y volver a medir— resultó innecesario porque la causa era otra (§2.9.3); los 5 directorios sin landing page (`/apps/`, `/books/`, `/landing-pages/`, `/lead-magnet/`, `/pages/` → `404`); decisión de owner sobre los 15 artefactos de demo/campaña publicados en el dominio de ingresos; CTA/checkout externos |
| P1-01 | `IN_PROGRESS` | `docs/seo-remediation-report.md` documenta la remediación de 2 descriptions, 9 hreflang y 10 canonical; `verify_tech_debt.py` queda en 0 errores. **2026-09-26: corregido el canonical de `blog/witchcraft-for-beginners-guide.html`, que apuntaba al host `www.cha0smagick.com` y sin `.html` — 7 auto-referencias reparadas (canonical, hreflang, og:url, JSON-LD `mainEntityOfPage`, y los 3 enlaces de compartir). Ver §11.2.** | Revisión manual de 180 missing alt/270 empty alt, heading skips, GSC/Bing, Rich Results, Lighthouse field y before/after. **Más, decidido el 2026-09-26:** `og:image`/`twitter:image` de ese mismo artículo apuntan a `img/blog/witchcraft-beginners-guide.webp`, que **no existe** — decisión de contenido del owner, no se cambió a ciegas |
...
### 11.2 Cierre local del 2026-09-26 — CLS de escritorio ARREGLADO + canonical corregido

Dueño: OpenCode (local). Alcance: solo lo que no depende de una plataforma externa. **Nada de esto marca una tarea como `DONE`**: P0-10 y P1-01 siguen `IN_PROGRESS` porque sus gates exigen evidencia en el dominio desplegado.

**1. Defecto de CLS — de "diagnosticado y registrado" a "arreglado y medido".** El diagnóstico anterior de §2.9.3 era incorrecto y quedó reemplazado por medición. Causa real: el patrón `<link rel="preload" as="style" onload="this.rel='stylesheet'">` en 371 archivos, que permite el primer pintado antes de aplicar la hoja de estilos. Detalle completo, refutación del popup, lección de ablación nula y resultados en §2.9.3.

Evidencia de aceptación local (`python -m http.server 8099` + `scripts/analytics/cls-shift-probe.mjs`, caché fría por perfil temporal nuevo de Puppeteer en cada corrida):

| Página | CLS antes | CLS después | Entradas antes → después |
|---|---|---|---|
| `index.html` (escritorio) | **1.09** | **0.000** | 8 → 0 (3 de 3 corridas) |
| `blog/index.html` (escritorio) | 0.000 | 0.000 | 0 → 0 |
| `blog/ai-machine-learning-paranormal-research.html` | **0.11** | **0.000** | 1 → 0 |
| `blog/all-24-elder-futhark-runes-complete-reference.html` | **0.26** | **0.000** | 1 → 0 |
| `blog/am-i-haunted-or-is-it-pareidolia.html` | **0.26** | **0.000** | 1 → 0 |
| `index.html` (móvil) | 0.000 | 0.000 | 0 → 0 (sin regresión) |
| `blog/am-i-haunted-or-is-it-pareidolia.html` (móvil) | 0.000 | 0.000 | 0 → 0 (sin regresión) |

Umbral CLS ≤ 0.1. La fila de `blog/index.html` está a propósito: medía 0 **sin** el arreglo, y por eso el plan **no** afirma "CLS 0 en las 371 páginas" sino "5 páginas medidas dan 0.000, 3 de las cuales fallaban antes".

**2. Alcance del cambio.** 371 archivos corregidos en disco = **370 versionados** + `checklist-ventas.html`, que está en `.gitignore:33` y por eso no aparece en `git status`. 622 HTML escaneados, 252 sin el patrón y sin tocar. Un reemplazo por archivo; ningún archivo cambió de versión del CSS; los `<noscript>` huérfanos de los 152 archivos que ya tenían un `<link rel="stylesheet">` real se dejaron intactos a propósito. Única otra modificación en el árbol: `docs/thin-articles-progress.md`, que es el timestamp que regenera `scripts/thin_articles_report.py` al correr la verificación.

**3. Sin regresión.** `python scripts/verify_tech_debt.py` → 569 páginas, 563 con JSON-LD, **0 errores de validación**, idénticos conteos de entidades (Article=467, BreadcrumbList=479, FAQPage=421, HowTo=314, WebApplication=61, SoftwareApplication=12, Product=36, Book=7) e imágenes (total=4183, lazy=68, sized=412, webp=9, raster=452); exit 0. `npm test` → **38 pytest + 229 Vitest en 11 archivos**; exit 0. `npm run build` → exit 0. `python scripts/thin_articles_report.py` → 467 artículos, 0 pendientes, 467 hechas; exit 0.

**4. Canonical corregido.** `blog/witchcraft-for-beginners-guide.html` apuntaba a `https://www.cha0smagick.com/blog/witchcraft-for-beginners-guide` — dominio equivocado, `www.` de más y ruta sin `.html`. Reparadas **7 auto-referencias** en una pasada: `rel="canonical"`, `rel="alternate" hreflang="en"`, `og:url`, JSON-LD `mainEntityOfPage.@id`, y los 3 enlaces de compartir (Facebook, Twitter, Pinterest). Ahora apuntan a `https://cha0smagicklabs.com/blog/witchcraft-for-beginners-guide.html`, que es exactamente lo que ya decía `sitemap.xml`. Barrido de todo el repo: solo 2 archivos contenían el host erróneo, el segundo es `projects/docs/archive/play-store-sales-research.md`, que es un documento archivado y no una página servida, así que no se tocó. Quedan 2 ocurrencias en el artículo, ambas `og:image`/`twitter:image` hacia `https://www.cha0smagick.com/img/blog/witchcraft-beginners-guide.webp`, un archivo que **no existe** — se reporta al owner en vez de sustituirlo en silencio, porque qué imagen social debe tener un artículo es una decisión de contenido.

**5. Artefactos nuevos.**
- `scripts/analytics/cls-shift-probe.mjs` — sonda de atribución de `layout-shift`. Solo mide, no cambia nada. Inyecta un `PerformanceObserver` por CDP `Page.addScriptToEvaluateOnNewDocument` (Puppeteer 25 quitó `evaluateOnNew*` de `Page.prototype`) y reporta por cada desplazamiento el nodo, su ascendencia, `previousRect`/`currentRect`, `innerWidth` vs `clientWidth` y si hay barra de desplazamiento. CLI: `--url`, `--viewport desktop|mobile`, `--wait`, `--block a,b` (ablación, con `blocked_request_count` para que una ablación nula sea detectable), `--json`. Exit 0 corrió / 1 falló / 2 no se observó ningún desplazamiento.
