...
**Límites que este audit NO cubre:** los datos de **campo** no se obtuvieron (PageSpeed Insights respondió `HTTP 429` incluso tras un reintento), así que el script marca `cwvComplete:false` e imprime un aviso de que la corrida no puede cerrar un gate de CWV pase lo que pase. **INP es immeasurable en laboratorio** y queda pendiente con independencia de las corridas. TBT es un proxy de laboratorio y **no es INP**. Laboratorio ≠ campo. Y el audit no dice absolutamente nada sobre ingresos, conversión ni revenue: una caída de performance es un coste, no una medición de negocio.
