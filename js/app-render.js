    ...
    orderedApps.forEach((app, index) => {
        ...
        card.innerHTML = `
            ...
            <div class="card-content">
                <h4>${app.name}${(app.id === 'psi-gym' || app.id === 'dream-machine') ? ' <span class="discount-badge">NEW!</span>' : ''}</h4>
                <p>${app.description}</p>
    ...
    detailsContainer.innerHTML = `
        <div class="detail-header-layout">
            ...
            <div class="detail-header-info">
                <h2>${item.name}${(item.id === 'psi-gym' || item.id === 'dream-machine' || item.id === 'codex-chaoticus-pdf' || item.id === 'tarot-chaos-pdf') ? ' <span class="discount-badge">NEW!</span>' : ''}</h2>
