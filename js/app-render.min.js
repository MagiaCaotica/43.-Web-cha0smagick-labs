...
${book.languageFlag.split(',').map(f => `<img src="https:
</div>` : ''}
<div class="card-footer">
<div class="status-container">
...
let actionButton = '';
if (item.hotmartLink) {
actionButton = `<a onclick="return false;" href="${item.hotmartLink}" class="hotmart-fb hotmart__button-checkout"><img src='https:
loadHotmartWidget();
} else if (item.url) {
...
${item.languageFlag.split(',').map(f => `<img src="https:
</div>` : ''}
...
renderAppsGrid();
renderBooksSection();
setTimeout(renderItemListSchema, 100); 
} else if (detailsView) {
const urlParams = new URLSearchParams(window.location.search);
const appId = urlParams.get('id');
renderAppDetails();
