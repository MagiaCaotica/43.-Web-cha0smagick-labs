 ...
 */
async function cli() {
    // Ensure fluent-ffmpeg can find the ffmpeg/ffprobe binaries shipped by ffmpeg-static
    try {
        const ffmpegStaticPath = require('ffmpeg-static') as string;
        process.env.FFMPEG_PATH = process.env.FFMPEG_PATH || ffmpegStaticPath;
        process.env.FFPROBE_PATH = process.env.FFPROBE_PATH || path.join(path.dirname(ffmpegStaticPath), 'ffprobe.exe');
    } catch (e) {
        console.warn('Could not resolve ffmpeg-static; falling back to PATH lookup for ffmpeg/ffprobe.');
    }
