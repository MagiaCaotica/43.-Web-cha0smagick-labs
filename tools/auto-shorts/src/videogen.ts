...
export class VideoGen {
    ...
    async generateVoice(options: tts.VoiceGenOptions) {
        ...
        // TTS disabled: write a silent WAV so downstream steps (combineVoiceFiles,
        // ffprobe/getAudioDuration, FFCreator audio) always receive a real audio file.
        await this.generateSilentAudio(options);
    }

    /**
     * Generate a silent WAV file when TTS is disabled.
     * Duration is derived from the text length (approximate speaking pace).
     */
    private async generateSilentAudio(options: tts.VoiceGenOptions) {
        const textLength = options.text?.length ?? 0;
        const duration = Math.min(120, Math.max(2, Math.ceil(textLength * 0.07)));
        ...
        await new Promise<void>((resolve, reject) => {
            fluent_ffmpeg()
                ...
                .on('end', () => resolve())
                .on('error', (err) => reject(new Error('Failed to generate silent audio: ' + err.message)))
                .run();
        });
    }
    ...
    async generateSubtitles(audio16kFile: string, srtFile: string, maxLen: number) {
        if (!this.internalOptions.disableSubtitles && !this.internalOptions.disableTTS) {
            return await WhisperSubtitles.transcribeSrt(this, audio16kFile, maxLen, srtFile, this.resPath);
        }

        // TTS disabled (no speech to transcribe) or subtitles disabled:
        // write an empty SRT so FFSubtitle always receives a valid subtitle file.
        fs.writeFileSync(srtFile, "");
    }
